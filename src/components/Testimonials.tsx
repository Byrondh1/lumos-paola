          ...
          {TESTIMONIALS.map((t, idx) => (
            <Animate key={t.name} animation="fade-up" delay={idx * 150}>
              <div className="relative p-7 rounded-3xl bg-brand-cream border border-brand-cream-dark hover:border-brand-peach/30 hover:shadow-lg hover:shadow-brand-peach/10 transition-all duration-300 flex flex-col h-full">
                ...
                {/* Stars */}
                <div className="flex gap-1 mb-5" aria-label="5 de 5 estrellas" role="img">
                  {[...Array(5)].map((_, i) => (
                    <StarIcon key={i} className="w-4 h-4 text-brand-peach" aria-hidden="true" />
                  ))}
                </div>
...
function StarIcon({ className, 'aria-hidden': ariaHidden }: { className?: string; 'aria-hidden'?: boolean | 'true' | 'false' }) {
  return (
